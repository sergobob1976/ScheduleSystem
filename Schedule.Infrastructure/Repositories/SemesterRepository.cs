...
public class SemesterRepository : ISemesterRepository
{
    ...
    {
        using var connection = CreateConnection();
        string sql = "SELECT * FROM `Semesters` WHERE Id = @Id";
        return await connection.QueryFirstOrDefaultAsync<Semester>(sql, new { Id = id });
    }

    public async Task<int> CreateAsync(Semester semester)
    {
        using var connection = CreateConnection();
        string sql = @"INSERT INTO `Semesters` (Name, StartDate, EndDate) 
    ...
    public async Task<bool> UpdateAsync(Semester semester)
    {
        ...
        string sql = @"UPDATE `Semesters` 
                       ...
                       WHERE Id = @Id";
        int rowsAffected = await connection.ExecuteAsync(sql, semester);
